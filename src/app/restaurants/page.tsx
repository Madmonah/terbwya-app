...
async function getRestaurants(): Promise<Restaurant[]> {
  try {
    ...
    const { data } = await supa
      .from('restaurants')
      .select('id,slug,name,description,city,district,cover_photo_url,rating,reviews_count,delivery_fee_egp,min_order_egp,avg_delivery_minutes,is_open,status,featured,lat,lng,offers:restaurant_offers(id,discount_percent,starts_at,ends_at),cuisine_category_id,cuisine_category:cuisine_categories(slug,name_ar,icon)')
      .eq('status', 'published')
